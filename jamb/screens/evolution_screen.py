from __future__ import annotations

from typing import TYPE_CHECKING

from textual.app import ComposeResult
from textual.containers import Horizontal, Vertical
from textual.screen import Screen
from textual.widgets import Footer, Label

from ..constants import EVOLUTION_MESSAGES, EVOLUTION_NAMES
from ..widgets.ascii_art import SnailArt

if TYPE_CHECKING:
    from ..app import JambApp


class EvolutionScreen(Screen):
    """One-time evolution announcement overlay."""

    def __init__(self, new_stage: str, **kwargs) -> None:
        super().__init__(**kwargs)
        self._new_stage = new_stage

    def compose(self) -> ComposeResult:
        app: JambApp = self.app  # type: ignore[assignment]
        state = app.state
        new_stage = self._new_stage

        spec = state.stats.highest() if new_stage == "adult" else None
        stage_names = EVOLUTION_NAMES.get(new_stage, {})
        form_name = stage_names.get(spec or "default", stage_names.get("default", new_stage.title()))

        with Vertical(classes="evolution-container"):
            yield Label("  [tomato bold]✦ ✦ ✦  EVOLUTION!  ✦ ✦ ✦[/]", classes="mt1")
            yield Label(f"  [lightskyblue]{EVOLUTION_MESSAGES[0]}[/]", classes="mt1")
            yield Label(f"  [lightskyblue]{EVOLUTION_MESSAGES[1]}[/]")
            with Horizontal(id="art-speech"):
                yield SnailArt(new_stage, spec, id="snail-art")
                yield Label("")
            yield Label(f"  [lightgreen bold]Jamb evolved into: {form_name}![/]", classes="mt1")
            yield Label(f"  Stage: {new_stage.title()}  |  Level {state.level}", classes="dim")
            yield Label("  [yellow bold]Press any key to continue[/]", classes="mt1")

        yield Footer()

    def on_key(self, event) -> None:
        app: JambApp = self.app  # type: ignore[assignment]
        app.show_main()
