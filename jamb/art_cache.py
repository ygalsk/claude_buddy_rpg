...
The statusline (bash) reads ~/.claude/jamb/art_cache.json instead of
...
CACHE_FILE = Path.home() / ".claude" / "jamb" / "art_cache.json"


def _art_fingerprint(state: JambState) -> tuple:
...
def render_art_cache(state: JambState) -> dict:
...
def write_art_cache(state: JambState) -> None:
